...
export interface RepositoryCategory {
    category: 'frontend' | 'backend' | 'mobile' | 'fullstack' | 'devops' | 'data-science' | 'other';
...
export class RepositoryAnalyzer {
     ...
     */
    private detectTechnologies(text: string, languages: any[]): {
        frontend: string[];
        backend: string[];
        mobile: string[];
        devops: string[];
        ...
        languages.forEach((lang: any) => {
            ...
            // Backend languages
            if (['python', 'java', 'go', 'rust', 'c#', 'php', 'ruby'].includes(langName)) {
    ...
    private categorizeRepository(
        ...
        if (scores.frontend > 0 && scores.backend > 0) {
            return {
                ...
                frameworks: this.extractFrameworks([...techs.frontend, ...techs.backend]),
            };
        }
    ...
    analyzeAllRepositories(repositories: any[]): {
        ...
        repositories.forEach(repo => {
            ...
            // Add to category
            const categoryKey = analysis.category === 'data-science' ? 'dataScience' : analysis.category;
