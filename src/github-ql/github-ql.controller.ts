...
export class GithubQlController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Repositories scanned successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns scan status and eligibility',
  })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Manually re-scan user repositories (with weekly limit)',
  })
  @ApiParam({ name: 'userId', description: 'User ID' })
  @ApiResponse({
    status: 200,
    description: 'Repositories re-scanned successfully',
  })
  ...
  @ApiOperation({
    summary:
      'Get intelligent analysis of user repositories (categorized by tech stack)',
  })
  @ApiParam({ name: 'username', description: 'GitHub username' })
  @ApiResponse({
    status: 200,
    description: 'Returns categorized repositories and tech stack analysis',
  })
  @ApiResponse({
    status: 404,
    description: 'No repositories found. Please scan first.',
  })
  async getIntelligentAnalysis(@Param('username') username: string) {
    return this.githubQlService.getIntelligentAnalysis(username);
  }

  @Post('verify-tech-stack')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Verify user claimed technologies against actual GitHub usage',
  })
  @ApiResponse({
    status: 200,
    description:
      'Returns verification results with matched/unmatched technologies',
  })
  @ApiResponse({
    status: 404,
    description: 'No repositories found. Please scan first.',
  ...
  async verifyTechStack(
    @Body() dto: { username: string; claimedTechnologies: string[] },
  ) {
    return this.githubQlService.verifyUserTechStack(
      dto.username,
      dto.claimedTechnologies,
    );
  }
}
